...
public class Time64HelperTest {
	...
	public void testNow() {
		...
		long time64 = Time64Helper.dateToTime64(now);
	...
	public void testFixedValue(long fixedValue) {
		...
		bb.rewind();
		long decodedFixedValue = bb.getLong();
		Date decodedDate = Time64Helper.time64ToDate(decodedFixedValue);
		long convertedFixedValue = Time64Helper.dateToTime64(decodedDate);
		log.debug("Decoded fixed date: " + new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS").format(decodedDate));
		assertEquals(fixedValue, decodedFixedValue);
		assertEquals(decodedFixedValue/1000, convertedFixedValue/1000);
	}
